...
class NexmoMessage {
	 ...
	 */
	public function displayOverview( $nexmo_response=null,$rand_key ){
		...
		foreach ( $info->messages as $message ) {
			...
			$message_status[] = $tmp;
			$status = $message_status[0]['status'];
			$id = $message_status[0]['id'];
			// die();
			// $sql = "UPDATE contacts SET status='$status' response_id ='$id' WHERE rand_key=$rand_key";
			$sql = "UPDATE contacts SET status='$status' WHERE rand_key=$rand_key";
